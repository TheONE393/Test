...
def chapter(chapter_id):
    ...
    if request.method == 'POST':
        ...
        new_attempt = Attempt(chapter_id=chapter_id, attempt_number=new_attempt_number)
...
def insert_sample_data():
    with app.app_context():
        ...
        answers = ['A', 'B', 'C', 'D', 'A']
        for i in range(5):
            question = Question(chapter_id=chapter.id, correct_answer=answers[i])
            db.session.add(question)
        db.session.commit()

if __name__ == '__main__':
    # insert_sample_data()  # Uncomment first run
    app.run(debug=True)